...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    public void paintComponent(Graphics g)  {
        ...
        if(p1.getLives()[1]&& !p1.win()){
            Image img5 = Toolkit.getDefaultToolkit().getImage("heart.PNG"); /*the image cannot be in the SRC folder*/
            g2.drawImage(img5,120 , 40 , 70 , 70 , this);

        }

        if(p1.getLives()[0] && !p1.win()){
            Image img6 = Toolkit.getDefaultToolkit().getImage("heart.PNG"); /*the image cannot be in the SRC folder*/
